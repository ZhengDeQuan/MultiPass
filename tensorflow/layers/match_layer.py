...
class MatchLSTMAttnCell(tc.rnn.LSTMCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            #tc.layers.fully_connected() #weights_initializer=initializers.xavier_initializer() #tf.contrib.layers.xavier_initializer(uniform=True,seed=None,dtype=tf.float32)
            #In uniform  x = sqrt(6. / (in + out)); [-x, x]
