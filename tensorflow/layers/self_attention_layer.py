...
def Mask(inputs, seq_len, mode='mul'):
    ...
    if seq_len == None:
        ...
        mask = tf.cast(tf.sequence_mask(seq_len), tf.float32) #如果seq_len.shape = [batch_size] , 则mask.shape = [batch_size , max_seq_len]
        for _ in range(len(inputs.shape)-2): # 4 - 2 = 2
            mask = tf.expand_dims(mask, 2)
        #mask [batch_size , max_seq_len] -> [batch_size , max_seq_len , 1] -> [batch_size , max_seq_len , 1 , 1]
        #inputs(A) [batch , sequence_lenK , sequence_lenQ , nb_head]
        #inputs(O) [batch , sequence_lenQ , nb_head * size_per_head]
        if mode == 'mul':
            return inputs * mask
        if mode == 'add':
...
def Multi_Head_Att(Q, K, V, nb_head, size_per_head, Q_len=None, V_len=None):
    '''
    :param Q: batch_size , sequence_len , hidden_size
    :param K:
    :param V:
    :param nb_head:
    :param size_per_head:
    ...
    :return:
    '''
    Q = tc.layers.fully_connected(Q, num_outputs = nb_head * size_per_head)
    Q = tf.reshape(Q, (-1, tf.shape(Q)[1], nb_head, size_per_head))#[batch, sequene_len , nb_head , size_per_head]
    Q = tf.transpose(Q, [0, 2, 1, 3])#[batch_size , nb_head , sequence_len , size_per_head]
    K = tc.layers.fully_connected(K,  num_outputs = nb_head * size_per_head)
    K = tf.reshape(K, (-1, tf.shape(K)[1], nb_head, size_per_head))
    K = tf.transpose(K, [0, 2, 1, 3])
    V = tc.layers.fully_connected(V, num_outputs = nb_head * size_per_head)
    V = tf.reshape(V, (-1, tf.shape(V)[1], nb_head, size_per_head))
    V = tf.transpose(V, [0, 2, 1, 3])
    # 计算内积，然后mask，然后softmax
    A = tf.matmul(Q, K, transpose_b=True) / tf.sqrt(float(size_per_head)) #[batch , nb_head , sequence_lenQ , sequence_lenK]
    A = tf.transpose(A, [0, 3, 2, 1]) # [batch , sequence_lenK , sequence_lenQ , nb_head]
    A = Mask(A, V_len, mode='add')
    A = tf.transpose(A, [0, 3, 2, 1]) # [batch , nb_head , sequence_lenQ , sequence_lenK]
    A = tf.nn.softmax(A)
    # 输出并mask
    O = tf.matmul(A, V) #K:key , V:value sequence_lenK == sequence_lenV [batch , nb_head , sequence_lenQ , size_per_head]
    O = tf.transpose(O, [0, 2, 1, 3]) #[batch , sequence_lenQ , nb_head , size_per_head]
    O = tf.reshape(O, (-1, tf.shape(O)[1], nb_head * size_per_head)) #[batch , sequence_lenQ , nb_head * size_per_head]
    O = Mask(O, Q_len, 'mul')
    return O