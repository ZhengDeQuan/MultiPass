...
class BRCDataset(object):
    ...
    def _one_mini_batch(self, data, indices, pad_id):
        ...
        for sample in batch_data['raw_data']:
            if 'answer_passages' in sample and len(sample['answer_passages']):
                ...
                start_id = min(padded_p_len -1 , sample['answer_spans'][0][0])#因为是0base的，所以长度如果为500的话，index最大为499
                ...
                if sequence_len - end_id - 1 < 0:
                    print("end_id = ",end_id)
                    print("gold_passage_offset = ",gold_passage_offset)
                    print("end_id_real = ",sample['answer_spans'][0][1])
                    print("******")
                    print("sequence_len = ",sequence_len)
                    print("max_passage_num = ",max_passage_num)
                    print("padded_p_len = ",padded_p_len)
                    exit(789)
                ...
                batch_data['sequence_label'].append(np.zeros(max_passage_num * padded_p_len))

        return batch_data

    def _zq_one_mini_batch(self, data, indices, pad_id):
        """
        Get one mini batch
        Args:
            data: all data
    ...
    def gen_mini_batches(self, set_name, batch_size, pad_id, shuffle=True):
        ...
        for batch_start in np.arange(0, data_size, batch_size):
            ...
            #yield self._zq_one_mini_batch(data, batch_indices, pad_id)
