...
class RCModel(object):
    ...
    def _decode_content(self):
        with tf.variable_scope('Answer_content_modeling_layer'):
            ...
            #[batch,5 * p,1] 完成w1T ReLU（W2v{passage} ）的操作
            self.content_probs = tf.nn.softmax(logits , 1)#[batch, 5 * p , 1]在 5 * p，这个维度上，进行归一化
            self.yes_probs = self.content_probs[0:,0:,0]#[batch , 5 * p]

    def _compute_content_loss(self):
        def sparse_null_loss(probs , labels , epsilon=1e-9 , scope = None):
            with tf.name_scope(scope,"log_loss"):
                ones = tf.ones_like(labels)
                losses = labels * tf.log(probs + epsilon) + (ones - labels) * tf.log(ones - probs + epsilon)
                loss = - tf.reduce_mean(losses)
            return loss

        with tf.variable_scope('verification_loss'):
            batch_size =  tf.shape(self.start_label)[0]
            content_probs = tf.reshape(self.content_probs , shape = [batch_size , -1]) # (batch , 5 * p , 1) --> (batch  , 5 * p)
    ...
    def _decode_verification(self):
        with tf.variable_scope("Cross_passage_verification"):
            batch_size = tf.shape(self.start_label)[0]
            content_probs = tf.reshape(self.content_probs , [tf.shape(self.p_emb)[0] , tf.shape(self.p_emb)[1], 1]) # [batch * 5 , p , 1]
            ...
            S = tf.matrix_set_diag(input = S , diagonal = tf.zeros(shape = [batch_size ,  tf.shape(S)[1] ] ,dtype = S.dtype)) #[batch , 5 , 5] except for the main digonal of innermost matrices is all 0
            ...
            g = tc.layers.fully_connected(RA_concated , num_outputs= self.max_p_num, activation_fn=None)#[batch , 5 ,1]
    ...
    def _compute_loss(self,b1=1.0,b2=0.5,b3=0.5):
        #total = 1.0 * (b1  + b2  + b3)
        #b1 = 1.0 * b1 / total
        #b2 = 1.0 * b2 / total
        #b3 = 1.0 * b3 / total
        with tf.variable_scope("joint_loss"):
            self.loss = b1 * self.boundary_loss + b2 * self.content_loss
            self.all_params = tf.trainable_variables()
            if self.weight_decay > 0:
                with tf.variable_scope('l2_loss'):
                    l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in self.all_params])
    ...
    def _train_epoch(self, train_batches, dropout_keep_prob):
        ...
        for bitx, batch in enumerate(train_batches, 1):
            ...
            _, loss, p_allennlp , q_allennlp = self.sess.run([self.train_op, self.loss,self.p_emb,self.q_emb], feed_dict)
            #print(p_allennlp.shape)
            #print(q_allennlp.shape)
    ...
    def find_best_answer(self, sample, start_prob, end_prob, padded_p_len):#要想从全部的篇章中抽取的关键要修改的点是，有些地方是padded0的，所以需要把这些地方的概率转化为0才行。
        """
